...
CodeMirror.defineMode("nujel", function () {
    ...
    function pushStack(state, indent, type) {
        ...
        state.gensymCounter = gensymCounter;
    }
    ...
    return {
        ...
        token: function (stream, state) {
            gensymCounter = state.gensymCounter;
                    ...
                    if (ch == "\"") {
                      ...
                      var keyWord = ''; var indentTemp = stream.column(), letter;
                        ...
                        */
                        ...
                        const curSym = state.indentStack ? state.indentStack.sym : 0;
                        const curDepth = state.indentStack ? state.indentStack.depth % 4 : 0;
                        returnType = `${BRACKET} depth-${curDepth} bracket-sym-${curSym}`;
                    } else if (ch == ")" || ch == "]") {
                        const curSym = state.indentStack ? state.indentStack.sym : 0;
                        const curDepth = state.indentStack ? state.indentStack.depth % 4 : 0;
                        returnType = `${BRACKET} depth-${curDepth} bracket-sym-${curSym}`;
