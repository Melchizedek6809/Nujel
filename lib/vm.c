...
static inline lVal lBufferViewRef(lVal car, size_t i){
...
static inline lVal lGenericRef(lVal col, lVal key){
	switch(col.type){
	case ltPair: {
...
static inline lVal lBufferViewSet(lVal car, size_t i, lVal v){
	const void *buf = car.vBufferView->buf->buf;
	const size_t length = car.vBufferView->length;
	const lBufferViewType viewType = car.vBufferView->type;
...
static inline lVal lGenericSet(lVal col, lVal key, lVal v){
	switch(col.type){
	case ltBytecodeArr: {
		const lBytecodeArray *arr = col.vBytecodeArr;
		lVal keyVal = requireNaturalInt(key);
