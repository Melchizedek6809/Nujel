...
"2F1A301807310D1A321A331A341807350D1A361A371A3818073901\n"
...
"0E001A010E021A030401040201\n"
"}#{##(module/insert/defer :random string/write (do (def *module* :random) (defclass rng \"A simple RNG\" :export (defn new (self seed) (when-not seed (set! seed (bit-xor (time) (time/milliseconds)))) (tree/new :seed seed :prototype* self)) (defn seed! (self seed) \"Set a new seed value for the RNG\" (set! self :seed seed)) (defn rng! (self) \"Generate a random integer\" (set! self :seed (+ 12345 (* (ref self :seed) 1103515245))) (bit-or (bit-shift-left (bit-and (ref self :seed) 65535) 16) (bit-and (bit-shift-right (ref self :seed) 16) 65535))) (defn int (self max) \"Return a value from 0 to MAX, or, if left out, a random int\" (if max (rem (abs (:rng! self)) max) (:rng! self))))))\n"
"0E001A010E021A030401040201\n"
...
"0E001A010E021A030401040201\n"
"}#{##(module/insert/defer :time string/write (do (def *module* :time) (defn seconds (timestamp) :export \"Return the seconds part of TIMESTAMP, defaults to current time\" (rem (or timestamp (time)) 60)) (defn minutes (timestamp) :export \"Return the minutes part of TIMESTAMP, defaults to current time\" (rem (div/int (or timestamp (time)) 60) 60)) (defn hours (timestamp) :export \"Return the hours part of TIMESTAMP, defaults to current time\" (rem (div/int (or timestamp (time)) 3600) 24))))\n"
"0E001A010E021A030401040201\n"
"}";