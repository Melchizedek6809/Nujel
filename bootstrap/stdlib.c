...
"} array/++ (a v) #@(documentation: \"Fills array a with value v\" name: array/fill!) #{##(array/length a len i v)\n"
"0E000E01040107020D020007030D240900110D0E010E030E04370D0E03230503\n"
"0E030E021E0AFFED0D0E0101\n"
"} array/fill! (a b) #@(documentation: \"Append array A to array B\" name: array/append) #{##(:array type-of a b :type-error \"array/append expects two arrays as its arguments\" current-lambda array/allocate array/length ret i rl)\n"
...
"00070A0D240900140D0E090E0A0E020E0A2B370D0E0A23050A0E0A0E080E0204\n"
"011E0AFFE60D240D150E080E020401070A0D0E080E090401070B0D2409001D0D\n"
"0E090E0A0E030E0A0E080E020401262B370D0E0A020103050A0E0A0E0B1E0AFF\n"
"E1160D0E090101\n"
"} array/append (a) #@(documentation: \"Duplicate Array A\" name: array/dup) #{##(array/allocate array/length a ret i)\n"
"0E000E010E020401040107030D020007040D240900140D0E030E040E020E042B\n"
"370D0E042305040E040E010E0204011E0AFFE60D0E0301\n"
...
"} array/reduce (arr fun) #@(documentation: \"Map an array, (map) should be preferred\" name: array/map) #{##(array/length arr len i fun)\n"
"0E000E01040107020D020007030D240900180D0E010E030E040E010E032B0401\n"
"370D0E032305030E030E021E0AFFE60D0E0101\n"
"} array/map (arr pred) #@(documentation: \"Filter an array, (filter) should be preferred\" name: array/filter) #{##(ri array/length arr len array/allocate ret ai pred array/length!)\n"
"020007000D0E010E02040107030D0E040E03040107050D020007060D2409002C\n"
"0D0E070E020E062B04010B00180E050E000E020E062B370D0E00020125050009\n"
"0004240D0E062305060E060E031E0AFFD20D0E080E050E00040201\n"
...
"01\n"
"} array/equal? (arr val) #@(documentation: \"Append VAL to ARR\" name: array/push) #{##(array/length! arr array/length val)\n"
"0E000E0102010E020E0104012504020E020E0104010201260E033701\n"
...
"} array/heap-sort array/sort (arr start end) #@(documentation: \"Return a newly allocated array with the values of ARR from START to END\" name: array/cut) #{##(max start min array/length arr end array/allocate ret i)\n"
"0E0002000E01040205010D0E020E030E0404010E05040205050D0E060E000200\n"
"0E050E01260402040107070D0E0107080D240900190D0E070E080E01260E040E\n"
"082B370D0E0802010305080E080E051E0AFFE50D0E070101\n"
"} array/cut)\n"
"1A001A011A021707030D1A041A051A061707070D1A081A091A0A17070B0D1A0C\n"
...
"02372E0101\n"
"} $set/gen (v) #@(documentation: \"(v) -> (v) --- Resolve V and get the associated value on the stack\" name: $def/val cat: :bytecode internal: #t) #{##(:symbol type-of v :invalid-bc-op \"Can only get symbol\" current-lambda :literal)\n"
...
"} assemble/relocate-op (code sym-map pos out) #@(name: assemble/emit-relocated-ops) #{##(code :relocate assemble/relocate-op sym-map pos out ΓεnΣym-97 op type-of ΓεnΣym-98 :int :pair assemble/emit-relocated-ops)\n"
"0E00111A01200B00140E020E000E030E040E050404050409005F150E0007060D\n"
"2409004C0D0E061107070D150E080E07040107090D0E091A0A200B00120E050E\n"
...
"} literal-index->bytecode-op *max-literal-arr* (lit lit-arr) #@(name: assemble/add-literal) #{##(array/length lit-arr 65535 :out-of-bounds \"Can only store up to 255 literals per bytecode-arr\" code current-lambda not= type-of lit :pair i equal? literal-index->bytecode-op array/length!)\n"
...
"FFD70D24090004240D0E0E0E0102010E000E0104012504020D0E010E000E0104\n"
"010201260E09370D0E0D0E000E010401020126040101\n"
...
"} bytecompile/def (source env) #@(name: bytecompile/set!) #{##(length source ΓεnΣym-31 :symbol type-of :type-error \"(set!) needs a symbol name and a value as arguments\" env bytecompile* $set/val $set/gen :arity-error \"(set!) needs 2 or 3 arguments\")\n"
"150E000E0112040107020D0E020202200B003E1A030E040E012C0401200B0007\n"
...
"} bytecompile/apply (source env) #@(documentation: \"Compile the forms in source\" name: bytecompile* cat: :bytecode internal: #t) #{##(resolves? source env resolve op type-of ΓεnΣym-75 :lambda :native-function :pair :symbol #f ΓεnΣym-76 do bytecompile/do let* bytecompile/let* def bytecompile/def set! bytecompile/set! if bytecompile/if while bytecompile/while and bytecompile/and or bytecompile/or fn* bytecompile/fn* macro* bytecompile/macro* environment* bytecompile/environment* try bytecompile/try return bytecompile/return apply bytecompile/apply quote bytecompile/quote bytecompile/procedure :type-error \"Can't evaluate that\" current-lambda bytecompile/literal)\n"
...
"0C0A00060D1A130B00130E040E0E0E01120E0204022E020900270E1C0E041A1D\n"
...
"} whitespace? l #@(documentation: \"Turn the provided char codes into a string and return it\" name: from-char-code) #{##(buffer/allocate list/length l buf i :int type-of 255 #f :type-error \"(from-char-code) expects :int arguments from 0 to 255, not: \" current-lambda buffer/length buffer->string)\n"
"0E000E010E020401040107030D020007040D2409005D0D1A050E060E02110401\n"
...
"0E0211370D0E021205020D0E042305040E040E0C0E0304011E0AFF9D0D0E0D0E\n"
"03040101\n"
"} from-char-code)\n"
"1A001A011A021707030D1A041A051A061707070D1A081A091A0A17070B0D1A0C\n"
"1A0D1A0E17070F01\n"
...
"} fmt/arg-sym (expr arguments-used opts) #@(name: fmt/expr) #{##(:string type-of expr :format-error \"fmt needs a string literal as a first argument, since it is implemented as a macro\" current-lambda split \":\" split-expr argument \"\" #f format-spec tree/+= opts :expr-count arguments-used fmt/format-arg fmt/arg-sym read read-vals \"Format argument specifier contains more than a single atom\" fmt/valid-argument? \"Format argument specifier should be either an integer or a symbol\" :int array/length \"fmt numbered argument is out of bounds\")\n"
"1A000E010E020401200B0007240900101A031A040E020E0504002E042F0D0E06\n"
...
"00101A031A170E090E0504002E042F0D1A180E010E14110401200B003E0E1411\n"
"02001E0C0A00150D0E14110E190E100401210C0A00060D1A0B0B00131A031A1A\n"
...
"} (self char) #{##(meta self :buffer buf buffer/length :position buffer/length! 128 char meta!)\n"
"0E000E011A02040207030D0E040E0304010E000E011A0504022602011E0B0015\n"
"0E060E031A070E040E030401250402090004240D0E030E000E011A0504020E08\n"
"370D0E090E011A050E000E011A050402020125040301\n"
...
"0301\n"
"} trim (text) #@(documentation: \"Convert text into a version using only capitals or uppercase letters.\\n\\ntext: The input string\\n\\nReturns the resulting upper-case version\" name: upper-case cat: :string-manipulation tests: ((\"TESTING THE CAPITALIZATION!\" (upper-case \"Testing THE Capitalization!\"))) related: (capitalize lower-case)) #{##(buffer/allocate buffer/length text ret i upper-case-char buffer->string)\n"
"0E000E010E020401040107030D020007040D240900180D0E030E040E050E020E\n"
"042B0401370D0E042305040E040E010E0204011E0AFFE20D0E060E03040101\n"
"} upper-case (text) #@(documentation: \"Convert text into a version using only lowercase letters.\\n\\ntext: The input string\\n\\nReturns the resulting lower-case version\" name: lower-case cat: :string-manipulation tests: ((\"testing the capitalization!\" (lower-case \"Testing THE Capitalization!\"))) related: (capitalize upper-case)) #{##(buffer/allocate buffer/length text ret i lower-case-char buffer->string)\n"
"0E000E010E020401040107030D020007040D240900180D0E030E040E050E020E\n"
"042B0401370D0E042305040E040E010E0204011E0AFFE20D0E060E03040101\n"
"} lower-case (text) #@(documentation: \"Convert text into a capitalized version.\\n\\nThis is simplified by having the first character be an upper-case version, as well\\nas any character following a whitespace character, and all other characters be in lower-case.\\n\\ntext: The input string\\n\\nReturns the capitalized version\" name: capitalize cat: :string-manipulation tests: ((\"Testing The Capitalization!\" (capitalize \"Testing THE Capitalization!\"))) related: (lower-case upper-case)) #{##(buffer/allocate buffer/length text ret caps? i upper-case-char lower-case-char whitespace? buffer->string)\n"
"0E000E010E020401040107030D1B07040D020007050D240900360D0E030E050E\n"
"040B00080E060900050E070E020E052B0401370D0E080E020E052B04010B0007\n"
"1B0900041C05040D0E052305050E050E010E0204011E0AFFC40D0E090E030401\n"
"01\n"
