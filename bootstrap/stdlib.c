...
"} compile (name args body) #@(name: meta/parse/body cat: :compiler internal: #t) #{##(body source nil? not= :type-name :pair tree/new :name name meta v ΓεnΣym-3 deftest :tests :string :documentation cat string \"\\n\" :keyword ΓεnΣym-4 :inline :source :related :export-as :export :cat trim join map split)\n"
"0E0007010D240900090D0E011205010E020E011204010B00071C0900041B0C0B\n"
"00110D0E031A040E011104011A0504020AFFD90D0E061A070E08040207090D24\n"
"0900E70D0E0011070A0D151A040E0A0401070B0D0E0B1A05200B00230E0A111A\n"
"0C200B00140E091A0D0E0A120E091A0D2B14370900062405000900A70E0B1A0E\n"
"200B001C0E091A0F0E100E110E091A0F2B04011A120E0A0403370900860E0B1A\n"
"13200B007D150E0A07140D0E141A15200B00140E090E0A1B370D0E091A160E01\n"
"3709005A0E141A17200B001A0E091A170E002C0E091A172B14370D0E00120500\n"
"09003B0E141A18200B00140E091A190E002C370D0E001205000900220E141A1A\n"
"200B00140E091A1A0E002C370D0E001205000900090E090E0A1B371609000424\n"
"160D0E001205000E000AFF1A0D0E091A0F2B0B00260E091A0F0E1B0E1C0E1D0E\n"
"1E0E091A0F2B1A1204020E1B04021A120402040137090004240D0E090101\n"
...
"} let (v) #@(documentation: \"Coerce to boolean\" name: boolean source: ((deftest #t (boolean #t)) (deftest #f (boolean #nil)) (deftest #f (boolean #f)) (deftest #t (boolean 0)) (deftest #t (boolean 1)) (deftest #t (boolean 0.1)) (deftest #t (boolean \"\")) (deftest #t (boolean \"a\")) (if v #t #f)) cat: :boolean-operations tests: ((#t (boolean \"a\")) (#t (boolean \"\")) (#t (boolean 0.1)) (#t (boolean 1)) (#t (boolean 0)) (#f (boolean #f)) (#f (boolean #nil)) (#t (boolean #t))) inline: #t related: (not)) #{##(v)\n"
"0E000B00071B0900041C01\n"
"} boolean (v) #@(documentation: \"Return true if V is false\" name: not source: ((deftest #f (not #t)) (deftest #t (not #f)) (deftest #t (not (< 5 1))) (deftest #t (not (= #f #t))) (deftest #t (not (< 3 2))) (if v #f #t)) cat: :boolean-operations tests: ((#t (not (< 3 2))) (#t (not (= #f #t))) (#t (not (< 5 1))) (#t (not #f)) (#f (not #t))) inline: #t related: (boolean)) #{##(v)\n"
"0E000B00071C0900041B01\n"
"} not (α) #@(documentation: \"Returns its argument\" name: identity source: (α) inline: #t) #{##(α)\n"
"0E0001\n"
"} identity (p) #@(documentation: \"(car (car p))\" name: caar source: ((deftest #t (caar '((#t) #f))) (car (car p))) cat: :list-manipulation tests: ((#t (caar '((#t) #f)))) inline: #t related: (cdr car cadr)) #{##(p)\n"
"0E00111101\n"
"} caar (p) #@(documentation: \"(cdr (car p))\" name: cdar source: ((deftest #t (cdar '((#f . #t) #f))) (deftest (#t) (cdar '((#f #t) #f))) (cdr (car p))) cat: :list-manipulation tests: (((#t) (cdar '((#f #t) #f))) (#t (cdar '((#f . #t) #f)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E00111201\n"
"} cdar (p) #@(documentation: \"(cdr (cdr p))\" name: cddr source: ((deftest #t (cddr '(#f #f . #t))) (deftest (#t) (cddr '(#f #f #t))) (cdr (cdr p))) cat: :list-manipulation tests: (((#t) (cddr '(#f #f #t))) (#t (cddr '(#f #f . #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E00121201\n"
"} cddr (p) #@(documentation: \"(car (cdr (car p)))\" name: cadar source: ((deftest #t (cadar '((#f #t) #f))) (car (cdr (car p)))) cat: :list-manipulation tests: ((#t (cadar '((#f #t) #f)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E0011121101\n"
"} cadar (p) #@(documentation: \"(car (cdr (cdr p)))\" name: caddr source: ((deftest #t (caddr '(#f #f #t))) (car (cdr (cdr p)))) cat: :list-manipulation tests: ((#t (caddr '(#f #f #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E0012121101\n"
"} caddr (p) #@(documentation: \"(cdr (cdr (cdr p)))\" name: cdddr source: ((deftest #t (cdddr '(#f #f #f . #t))) (deftest (#t) (cdddr '(#f #f #f #t))) (cdr (cdr (cdr p)))) cat: :list-manipulation tests: (((#t) (cdddr '(#f #f #f #t))) (#t (cdddr '(#f #f #f . #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E0012121201\n"
"} cdddr (p) #@(documentation: \"(car (cdr (cdr (cdr p))))\" name: cadddr source: ((deftest #t (cadddr '(#f #f #f #t))) (car (cdr (cdr (cdr p))))) cat: :list-manipulation tests: ((#t (cadddr '(#f #f #f #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E001212121101\n"
"} cadddr (p) #@(documentation: \"(cdr (cdr (cdr (cdr p))))\" name: cddddr source: ((deftest #t (cddddr '(#f #f #f #f . #t))) (cdr (cdr (cdr (cdr p))))) cat: :list-manipulation tests: ((#t (cddddr '(#f #f #f #f . #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E001212121201\n"
"} cddddr (p) #@(documentation: \"(car (cdr (cdr (cdr (cdr p)))))\" name: caddddr source: ((deftest #t (caddddr '(#f #f #f #f #t))) (car (cdr (cdr (cdr (cdr p)))))) cat: :list-manipulation tests: ((#t (caddddr '(#f #f #f #f #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E00121212121101\n"
"} caddddr (p) #@(documentation: \"(cdr (cdr (cdr (cdr (cdr p)))))\" name: cdddddr source: ((deftest #t (cdddddr '(#f #f #f #f #f . #t))) (cdr (cdr (cdr (cdr (cdr p)))))) cat: :list-manipulation tests: ((#t (cdddddr '(#f #f #f #f #f . #t)))) inline: #t related: (cdr car caar)) #{##(p)\n"
"0E00121212121201\n"
...
"} not-equal? (val) #@(documentation: \"Test whether val is an integer\\n\\nInteger are sometimes called fixnum's as well\\n\\nval: This argument is going to be tested\\n\\nWhether val is an integer\" name: int? source: ((deftest #t (int? 123)) (deftest #f (int? 123.123)) (deftest #f (int? 'asd)) :inline (= :int (:type-name val))) cat: :type-predicate tests: ((#f (int? 'asd)) (#f (int? 123.123)) (#t (int? 123))) inline: #t) #{##(:int :type-name val)\n"
"1A001A010E0204012001\n"
"} int? (val) #@(documentation: \"Test whether val is a floating-point number\\n\\nFloat's are sometimes called flonum's as well\\n\\nval: This argument is going to be tested\\n\\nWhether val is a floating-point number\" name: float? source: ((deftest #f (float? 123)) (deftest #t (float? 123.123)) (deftest #f (float? 'abc)) :inline (= :float (:type-name val))) cat: :type-predicate tests: ((#f (float? 'abc)) (#t (float? 123.123)) (#f (float? 123))) inline: #t) #{##(:float :type-name val)\n"
"1A001A010E0204012001\n"
